...
import icu.sgwxm.JPADemo.dao.UserDao;
...
//    }
    ...
    public void testFindAll() {
        List<UserDO> userDOList = userDao.findAll(Sort.by(Sort.Direction.DESC,"account"));
    ...
    public void testFindByAccount(){
       UserDO userDO = userDao.findAccount("wentian");
        if (userDO != null) {
            System.out.println("name = " + userDO.getName());
            System.out.println("account = " + userDO.getAccount());
        }
    ...
    public void testFindOneByName(){
        UserDO userDO = userDao.findOne("东方不败",3L);
        if(userDO != null){
            System.out.println("name = " + userDO.getName());
            System.out.println("account = " + userDO.getAccount());
        }
    ...
    public void testFindOneByIds(){
        Long[] ids = {1L,2L};
        List<UserDO> byIdArr = userDao.findByIdArr(ids);
        if(byIdArr.size() != 0){
            for (UserDO userDO : byIdArr) {
                System.out.println("name = " + userDO.getName());
                System.out.println("account = " + userDO.getAccount());
            }
        }
    ...
    public void testFindUserAll(){
        List userAll = userDao.findUserAll();
        System.out.println(userAll.get(1));
    }
